...
const char * const BITCOIN_CONF_FILENAME = "aurumcoinhunter.conf";
const char * const BITCOIN_PID_FILENAME = "aurumcoinhunterd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "aurumcoinhunter" is a composite category enabling all AurumCoinHunter-related debug output
                if(ptrCategory->count(std::string("aurumcoinhunter"))) {
...
#else
    const char* pszModule = "aurumcoinhunter";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AurumCoinHunter
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AurumCoinHunter
    // Mac: ~/Library/Application Support/AurumCoinHunter
    // Unix: ~/.aurumcoinhunter
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AurumCoinHunter";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AurumCoinHunter";
#else
    // Unix
    return pathRet / ".aurumcoinhunter";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty aurumcoinhunter.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override aurumcoinhunter.conf
