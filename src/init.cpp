...
void PrepareShutdown()
{
    ...
    RenameThread("aurumcoinhunter-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                                  "aurumcoinhunter (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all AurumCoinHunter specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N AU79 anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://aurumcoinhunter.org>";
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("aurumcoinhunter-loadblk");
...
/** Sanity checks
 *  Ensure that AurumCoinHunter Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("AurumCoinHunter Core version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single AurumCoinHunter Core process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    //lite mode disables all AurumCoinHunter-specific functionality
    fLiteMode = GetBoolArg("-litemode", false);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, all AurumCoinHunter-specific functionality is disabled."));
    }
    ...
    // ********************************************************* Step 11c: update block tip in AurumCoinHunter modules
    ...
    // ********************************************************* Step 11d: start aurumcoinhunter-ps-<smth> threads
