...
*
*   Why is this needed?
*   - In AurumCoinHunter some blocks are superblocks, which output much higher amounts of coins
...
{
    if(fLiteMode) return; // disable all AurumCoinHunter specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f AU79\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
